...
function generateComprehensiveGuidance(userProfileText: string): string {
    ...
    if (topCareer.localInstitutions && topCareer.localInstitutions.length > 0) {
        response += `**Employability Hub:** ${topCareer.localInstitutions.slice(0, 2).join(", ")} (Signals high competence)\n\n`;
    }

    response += `### B. 90-Day Execution Roadmap\n`;
    response += `- **Foundation (Days 1-30):** Master ${topCareer.requiredSkills?.[0] || 'Core Skills'}. Setup your "Proof Portfolio".\n`;
    response += `- **Verification (Days 31-60):** Building Phase. Complete one tangible project or internship in ${location}.\n`;
    response += `- **Entry (Days 61-90):** Market Assault. Apply with your proof, not just your CV.\n\n`;

    if (matches.length > 0) {
        response += `### C. Alternative Smart Paths\n`;
        matches.forEach(m => {
            response += `- **${m.title}**: High growth potential in ${m.industry}.\n`;
        });
        response += `\n`;
    }

    response += `**ACCOUNTABILITY:** This plan only works if you prioritize *shipping* over *watching*. If you haven't taken your first action within 48 hours, you have already failed this path.\n\n`;
    response += `**NEXT IMMEDIATE ACTION:** Research admission/entry for ${topCareer.localInstitutions?.[0] || 'the nearest training center'} or start your first project today.`;
...
export async function matchCareers(answers: QuizAnswer[]): Promise<CareerMatchResult> {
    ...
    const recommendations: CareerRecommendation[] = topCareers.map(({ career, score }) => ({
        ...
        whyFits: `Matches your interest in ${career.industry} and your specific background.`,
 ...
 * Generates a high-quality cover letter without AI API calls.
 */
export function generateLocalCoverLetter(profile: any, job: any, tone: string): string {
    ...
    // Rich data context
    const impact = profile.impact_metrics ? `My track record includes demonstrated results such as ${profile.impact_metrics}.` : "";
    const leadership = profile.leadership_experience ? `Driven by a leadership mindset developed while ${profile.leadership_experience}, I aim to bring that same initiative to your team.` : "";
    const hook = profile.unique_hook ? `What sets me apart is ${profile.unique_hook}.` : "";

    const templates = {
        formal: `Dear Hiring Manager,

I am writing to formally express my interest in the ${role} position at ${company}. Having followed ${company}'s contributions to the Sierra Leonean market, I am impressed by your organization's commitment to excellence and professional standards.

With my background in ${skill}, I bring a dedicated and credential-driven approach to this role. ${impact} ${leadership} I am confident that my experience aligns well with the requirements of your team and I am eager to contribute to your continued success.

${hook} Thank you for your time and consideration. I look forward to the possibility of discussing how my skills can benefit ${company}.

Sincerely,
${name}`,

        dynamic: `Hello ${company} Team,

I'm reaching out because I saw the ${role} opening and immediately knew my trajectory aligned with your mission. I don't just fill roles; I solve problems and drive results.

${hook} My expertise in ${skill} is exactly what's needed to hit the ground running at ${company}. ${impact} ${leadership}

I'm an agile professional who thrives on challenge and speed, and I'm ready to bring that energy to your next big project. Let's talk about how I can help ${company} reach its next milestone.

Best,
${name}`,

        academic: `To the Selection Committee,

It is with great enthusiasm that I submit my application for the ${role} position at ${company}. My academic journey, centered around ${skill}, has prepared me to engage deeply with the rigors of this institution.

${impact} I am particularly drawn to ${company} because of your reputation for intellectual excellence. ${leadership} ${hook}

I believe my research-oriented mindset and pedagogical philosophy would make me a valuable addition to your academic community. I look forward to discussing my potential contributions to ${company} in further detail.

Regards,
${name}`,

        standard: `Dear ${company} Team,

I am writing to apply for the ${role} position I recently saw advertised. My experience and skills, particularly in ${skill}, make me a strong candidate for this opportunity.

${impact} I have a proven track record of professional commitment and I am eager to bring my skills to a forward-thinking organization like ${company}. ${leadership}

${hook} I am confident that my background will allow me to contribute effectively to your team from day one. Thank you for considering my application. I look forward to hearing from you.
