                    ...
                    >
                        <Card className="w-[350px] sm:w-[400px] h-[550px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 overflow-hidden rounded-[2rem]">
                            ...
                            <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-slate-50/50 dark:bg-slate-900/50 relative">
                                {!isProfileComplete && (
                                    <div className="bg-amber-50 border border-amber-100 p-3 rounded-xl mb-4 flex items-center gap-2">
                                        <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
                                        <p className="text-[10px] text-amber-800 font-bold uppercase tracking-tight">
                                            Limited Mode: Complete your profile for better precision.
                                        </p>
                                    </div>
                                )}
                            ...
                            <div className="p-4 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 shrink-0 space-y-4">
                                ...
                                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide no-scrollbar">
                                    {quickActions.map((action, i) => (
                                        <button
                                            ...
                                            className="whitespace-nowrap px-4 py-1.5 rounded-full bg-slate-50 hover:bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500 transition-colors border border-slate-100"
                                        >
                                            {action.label}
                                        </button>
                                    ))}
                                ...
                                <form onSubmit={handleSubmit} className="flex gap-2">
                                    <input
                                        className="flex-1 px-5 py-3 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none focus:ring-2 focus:ring-[#1E5EFF]/10 outline-none text-sm transition-all font-medium"
                                        value={input}
