                    ...
                    <div className="relative z-20 w-full p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-10">
                        <div className="space-y-8 max-w-3xl">
                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#1FA774]/20 border border-[#1FA774]/30 rounded-full backdrop-blur-md">
                                ...
                                <span className="text-[#4ADE80] font-black text-[10px] uppercase tracking-[0.2em]">Neural Aptitude Protocol</span>
                            </div>
                            <h1 className="text-4xl md:text-7xl font-black text-white leading-[0.9] tracking-tight font-poppins uppercase">
                                Aptitude <br /> <span className="text-gradient-salone brightness-125">Mapping</span> Hub
                            </h1>
                            <p className="text-lg md:text-xl text-slate-300 font-medium font-inter max-w-2xl leading-relaxed border-l-4 border-[#1FA774] pl-6 italic">
                            ...
                            {step === 0 && (
                                <div className="pt-4">
                                    <Button
                                        onClick={() => setStep(1)}
                                        className="h-20 px-12 rounded-3xl bg-[#1FA774] hover:bg-[#1E5EFF] text-white gap-6 text-xl font-black uppercase tracking-widest shadow-2xl shadow-emerald-500/20 active:scale-95 group transition-all"
                                    >
                                        Initialize Assessment
                                        <Zap className="h-6 w-6 group-hover:rotate-12 transition-transform text-[#4ADE80]" />
                                    </Button>
                                </div>
                            )}
                        </div>
                ...
                <div className="mt-8">
                    {step === 1 && (
                        <div className="max-w-4xl mx-auto space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
                            <div className="space-y-6">
                                <div className="flex justify-between items-end">
                                    <div className="space-y-2">
                                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#1FA774] italic">// Sequential Progress</p>
                                        <div className="flex items-baseline gap-4">
                                            <span className="text-7xl font-black text-[#0B1F3A] font-poppins leading-none tracking-tighter italic">{currentQuestion + 1}</span>
                                            <span className="text-2xl font-bold text-slate-300 font-inter italic">OF {QUESTIONS.length}</span>
                                        </div>
                                    </div>
                                    <div className="text-right space-y-3">
                                        <Badge className="bg-[#1E5EFF]/5 text-[#1E5EFF] border border-[#1E5EFF]/10 font-black uppercase tracking-widest text-[9px] px-4 py-2 rounded-full italic">
                                        ...
                                        <div className="flex justify-end">
                                            <Button
                                                ...
                                                className="text-slate-300 hover:text-red-500 font-black text-[10px] uppercase tracking-[0.3em] h-10 italic"
                                            >
                            ...
                            <Card className="p-16 md:p-24 shadow-2xl rounded-[4rem] border-none bg-slate-50 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-12 opacity-5 rotate-12 group-hover:rotate-0 transition-transform">
                                    <Sparkles className="w-64 h-64 text-[#0B1F3A]" />
                                </div>

                                <h3 className="text-3xl md:text-5xl font-black text-[#0B1F3A] mb-20 text-center leading-[1] tracking-tighter font-poppins italic uppercase max-w-2xl mx-auto">
                                    "{QUESTIONS[currentQuestion].text}"
                                </h3>

                                <div className="grid gap-4 max-w-xl mx-auto relative z-10">
                                    {[
                                        { label: "Absolutely True", value: 4, color: "bg-[#0B1F3A] text-white hover:bg-[#1FA774] shadow-2xl shadow-[#0B1F3A]/30" },
                                        { label: "Mostly True", value: 3, color: "bg-white text-[#0B1F3A] border-4 border-slate-100 hover:border-[#1E5EFF] hover:text-[#1E5EFF]" },
                                        { label: "Somewhat / Maybe", value: 2, color: "bg-white text-[#0B1F3A] border-4 border-slate-100 hover:border-slate-300" },
                                        { label: "Not Really", value: 1, color: "bg-white text-slate-400 border-4 border-slate-50 hover:bg-slate-100" },
                                        { label: "Not At All", value: 0, color: "bg-red-50 text-red-500 border-4 border-red-100 hover:bg-red-500 hover:text-white" }
                                    ].map((option, i) => (
                                        <button
                                            key={i}
                                            onClick={() => handleAnswer(option.value)}
                                            className={`w-full h-20 rounded-3xl font-black uppercase tracking-[0.2em] text-xs transition-all active:scale-[0.95] flex items-center justify-center italic ${option.color}`}
                                        >
                                            {option.label}
                                        </button>
                                    ))}
                    ...
                    {step === 2 && (
                        <div className="text-center py-40 space-y-12 animate-in fade-in duration-1000">
                            ...
                            <div className="space-y-4">
                                <h2 className="text-4xl md:text-5xl font-black text-[#0B1F3A] tracking-tighter uppercase italic leading-none font-poppins">Synthesizing Profile...</h2>
                    ...
                    {step === 3 && results && (
                        <div id="results" ref={resultsRef} className="space-y-24 animate-in fade-in zoom-in-95 duration-1000 pb-20">
                            <div className="text-center space-y-8">
                                ...
                                <h2 className="text-6xl md:text-9xl font-black text-[#0B1F3A] tracking-tighter leading-none font-poppins uppercase italic">Top Matches.</h2>
                            ...
                            <div className="grid gap-16">
                                {results.careers.map((career: any) => (
                                    <Card key={career.rank} className="group overflow-hidden rounded-[4rem] border-none shadow-2xl hover:shadow-[#1E5EFF]/10 transition-all duration-700 bg-white">
                                        <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
                                            <div className="p-12 lg:w-2/5 space-y-10 bg-slate-50/50">
                                                <div className="space-y-6">
                                                    ...
                                                    <h3 className="text-4xl md:text-5xl font-black text-[#0B1F3A] leading-[1] tracking-tighter uppercase font-poppins italic">{career.career_name}</h3>
                                        ...
                                        <div className="bg-[#0B1F3A] p-10 flex flex-col sm:flex-row items-center justify-between gap-8 overflow-hidden relative">
                                            ...
                                            <div className="relative z-10 space-y-2">
                                                <p className="text-white font-black text-3xl italic uppercase tracking-tighter font-poppins leading-none">Activate this career path?</p>
                            ...
                            <div className="flex flex-col items-center gap-10 pt-10">
                                <div className="bg-slate-50 p-16 rounded-[4rem] border border-slate-100 shadow-2xl max-w-3xl text-center space-y-10 group relative overflow-hidden">
                                    ...
                                    <h4 className="text-3xl font-black text-[#0B1F3A] uppercase tracking-tighter italic font-poppins relative z-10 leading-none">Immediate <br /><span className="text-[#1FA774]">Tactical Next Steps:</span></h4>
