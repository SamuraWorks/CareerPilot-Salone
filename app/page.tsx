  ...
  return (
    <main className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 selection:bg-primary/20 Outfit">
      ...
      <section className="relative flex-1 flex flex-col items-center justify-center py-20 md:py-32 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 bg-gradient-to-br from-[#3b82f6] to-[#10b981] opacity-100" />
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            {/* Main Headline (Exactly as Screenshot) */}
            <motion.h1
              variants={itemVariants}
              className="text-[44px] leading-[1.1] md:text-[84px] font-black text-white tracking-tight"
            >
              Navigate Your Career <br />
              Path in <span className="inline-block px-5 py-1 bg-[#1fb65e] rounded-[1.5rem] mt-2 md:mt-0">Sierra Leone</span>
            </motion.h1>

            {/* Subheadline (Exactly as Screenshot) */}
            <motion.p
              variants={itemVariants}
            ...
            {/* CTA Buttons (Exactly as Screenshot) */}
            <motion.div
              variants={itemVariants}
            ...
            >
              ...
              <Link href="/aptitude-test" className="w-full sm:w-auto">
            ...
            <motion.div variants={itemVariants} className="pt-16 flex flex-col items-center gap-4">
              <div className="flex -space-x-2">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="w-10 h-10 rounded-full border-2 border-[#3b82f6] bg-slate-200 bg-cover bg-center" style={{ backgroundImage: `url(https://i.pravatar.cc/100?img=${i + 20})` }} />
                ))}
              </div>
      ...
      <section className="py-24 bg-white dark:bg-slate-950">
        <div className="container mx-auto px-6">
          <div className="text-center space-y-4 mb-20 text-slate-900 border-b border-slate-100 pb-12">
            <h2 className="text-[32px] md:text-5xl font-black tracking-tight leading-tight">Everything You Need to <br /> Build Your Career</h2>
            <p className="text-slate-500 font-medium max-w-xl mx-auto">CareerPilot Salone provides comprehensive tools to help you discover, plan, and pursue your ideal career path.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: "Career Test", icon: FileText, color: "text-blue-500", bg: "bg-blue-50" },
              { title: "Roadmaps", icon: Compass, color: "text-emerald-500", bg: "bg-emerald-50" },
              { title: "AI Advisor", icon: Brain, color: "text-purple-500", bg: "bg-purple-50" },
              { title: "Opportunities", icon: Briefcase, color: "text-orange-500", bg: "bg-orange-50" }
            ].map((f, i) => (
              <div key={i} className="p-10 rounded-[2.5rem] bg-slate-50 border border-slate-100 group hover:bg-white hover:shadow-2xl hover:shadow-slate-200 transition-all cursor-pointer">
      ...
      <footer className="py-12 border-t border-slate-100 bg-white">
        <div className="container mx-auto px-6 flex flex-col items-center gap-6">
          <Image src="/logo.png" alt="Logo" width={140} height={40} className="opacity-30 mix-blend-multiply" />
