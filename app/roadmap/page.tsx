...
function RoadmapContent() {
  ...
  // Handle incoming 'target' from Career Explorer
  useEffect(() => {
    if (targetId && !displayedRoadmap && !isLoading) {
      const career = SIERRA_LEONE_CAREERS.find(c => c.id === targetId);
      if (career) {
        setQuery(career.title);
        // We need a slight delay to ensure the query state is set or just call handleGenerate directly with the title
        ...
        <section className="relative rounded-3xl overflow-hidden min-h-[400px] flex items-center shadow-sm bg-slate-50">
          ...
          <div className="relative z-20 w-full p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 h-full">
            <div className="space-y-6 max-w-2xl bg-white/70 backdrop-blur-md p-8 md:p-12 rounded-[2.5rem] border border-white/50 shadow-xl">
              ...
              <h1 className="text-4xl md:text-5xl font-black leading-tight font-sans tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-blue-700">
                Roadmaps
              </h1>
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center gap-3">
                        <Badge className="bg-blue-50 text-blue-600 border-none font-bold px-3 py-1 uppercase text-[10px] tracking-wider">High Value Path</Badge>
                        <Badge className="bg-slate-100 text-slate-500 border-none font-bold px-3 py-1 uppercase text-[10px] tracking-wider">
                          Automation Risk: {displayedRoadmap.why_this_career.growth_outlook.split('Risk: ')[1] || 'Low'}
                        </Badge>
                      </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  <Card className="p-6 rounded-[2rem] bg-amber-50/50 border border-amber-100/50 hover:bg-amber-50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-black text-[#0B1F3A] uppercase tracking-tight text-xs">Market Reality</h3>
                    </div>
                    <p className="text-xs text-slate-700 font-medium leading-relaxed italic">
                      "{displayedRoadmap.career_details.market_reality}"
                    </p>
                  </Card>
                  ...
                  <Card className="p-6 rounded-[2rem] bg-blue-50/50 border border-blue-100/50 hover:bg-blue-50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-black text-[#0B1F3A] uppercase tracking-tight text-xs">Skill Gaps</h3>
                    </div>
                    <p className="text-xs text-slate-700 font-medium leading-relaxed italic">
                      "{displayedRoadmap.career_details.skill_gap_analysis}"
                    </p>
                  </Card>
                  ...
                  <Card className="p-6 rounded-[2rem] bg-indigo-50/50 border border-indigo-100/50 hover:bg-indigo-50 transition-colors col-span-1 lg:col-span-2">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-black text-[#0B1F3A] uppercase tracking-tight text-xs">Your Pathway Strategy</h3>
                    </div>
                    <p className="text-xs text-slate-700 font-medium leading-relaxed italic mb-2">
                      Based on your profile, this roadmap is designed as a:
                    </p>
                    <div className="flex items-center gap-2">
                      <Badge className="bg-indigo-600 hover:bg-indigo-700 text-white border-none text-[10px] uppercase font-bold px-3 py-1">
                        {displayedRoadmap.career_details.skill_gap_analysis.toLowerCase().includes("qualified") ? "Direct Advancement Plan" : "Fast-Track Catch-Up Plan"}
                      </Badge>
                      <span className="text-[10px] text-indigo-600 font-bold uppercase tracking-wider">
                        Target: {displayedRoadmap.career_details.skill_gap_analysis.toLowerCase().includes("qualified") ? "Promotion / specialization" : "Qualification in 90 Days"}
                      </span>
                    </div>
                  </Card>
                </div>
                      ...
                      {displayedRoadmap.universities && displayedRoadmap.universities.length > 0 ? (
                        displayedRoadmap.universities.map((uni, i) => (
                          <div key={i} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                            <p className="font-black text-[#0B1F3A] text-xs">{uni.name}</p>
                            <p className="text-[10px] text-slate-500 font-bold uppercase">{uni.focus}</p>
                          </div>
                        ))
                      ) : (
                  ...
                  <Card className="p-6 rounded-[2rem] bg-slate-50 border border-slate-100 hover:bg-white hover:shadow-md transition-all">
                    ...
                    <div className="space-y-3">
                      {displayedRoadmap.mentors && displayedRoadmap.mentors.map((m, i) => (
                        <div key={i} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                          <p className="font-black text-[#0B1F3A] text-xs">{m.role}</p>
                          <p className="text-[10px] text-slate-500 italic leading-snug">"{m.help_with}"</p>
                        </div>
                      ))}
                    </div>
                  </Card>
                          ...
                          <Card className={cn(
                            ...
                            <div className="mb-6 space-y-2">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {phase.what_you_will_learn.slice(0, 3).map((skill, sIdx) => (
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12">
              ...
              <div className="inline-block mb-4 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-full">
                <span className="text-emerald-700 font-bold text-xs uppercase tracking-widest">Strategy Hub: Roadmap Engine</span>
              </div>
              <h1 className="text-5xl font-black mb-4 text-[#0B1F3A] tracking-tight uppercase">Roadmaps</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 max-w-4xl mx-auto text-left">
                <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-8 hover:shadow-md transition-all">
                  ...
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Week Sprints</p>
                </div>
                <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-8 hover:shadow-md transition-all">
                  <div className="flex items-center justify-center gap-4 mb-3">
                  ...
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Powered Logic</p>
                </div>
                <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-8 hover:shadow-md transition-all">
                  <div className="flex items-center justify-center gap-4 mb-3">
                  ...
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Sectors Covered</p>
                </div>
              </div>
            </div>
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-[#F9FAFB] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-bold animate-pulse">Initializing Intelligence Hub...</p>
        </div>
      </div>
    }>
      <RoadmapContent />
