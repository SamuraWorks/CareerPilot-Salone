...
import { Search, MapPin, Briefcase, Clock, ExternalLink, BookOpen, Bookmark, Loader2 } from "lucide-react"
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-emerald-50 via-background to-background border-none shadow-md">
          <div className="flex flex-col md:flex-row items-center">
            <div className="p-8 flex-1">
              <h1 className="text-3xl md:text-4xl font-black mb-4 tracking-tight">Jobs & Training</h1>
              <p className="text-muted-foreground text-lg max-w-lg">
                Explore real opportunities in Freetown, Bo, and beyond. Your next big career move starts here in 🇸🇱.
              </p>
            </div>
            <div className="relative w-full md:w-80 h-48 md:h-64 shrink-0">
              <Image
          ...
          <TabsContent value={selectedType} className="mt-6 space-y-4">
            {loading ? (
              <div className="flex flex-col items-center justify-center p-20 text-muted-foreground">
                ...
                <p className="font-medium">Finding the best opportunities for you...</p>
              </div>
              ...
              <Card className="p-12 text-center bg-slate-50 border-dashed border-2">
                ...
                <Button variant="link" onClick={() => { setSearchTerm(""); setSelectedType("all") }} className="mt-2">
                  Clear all filters
                </Button>
              </Card>
            ) : (
              filteredOpportunities.map((opp) => (
                <Card key={opp.id} className="p-6 hover:shadow-lg transition-shadow border-slate-100 group">
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                          <h3 className="text-xl font-bold text-slate-900 group-hover:text-primary transition-colors">{opp.title}</h3>
                          <div className="flex gap-2">
                            <Badge variant="outline" className="bg-slate-50 uppercase text-[10px] font-black border-slate-200">
                              {opp.type}
                            </Badge>
                            {opp.salary && (
                              <Badge className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border-none font-black text-[10px] uppercase">
                                {opp.salary}
                              </Badge>
                            )}
                          </div>
                        </div>
                        <p className="font-bold text-slate-600 mt-0.5">{opp.organization}</p>
                      </div>
                    </div>
                    ...
                    {/* Description */}
                    <p className="text-slate-600 line-clamp-2 text-sm leading-relaxed">{opp.description}</p>

                    {/* Actions */}
                    <div className="flex flex-col sm:flex-row gap-3 pt-2">
                      <Button onClick={() => handleApply(opp.link, opp.title)} className="flex-1 gap-2 font-black uppercase text-xs tracking-wider h-12 shadow-lg shadow-primary/20">
                        View Details & Apply
                      ...
                      <Button
                        ...
                        className={cn("gap-2 h-12 font-bold px-6 border border-slate-200", savedOpportunities.includes(opp.id) && "bg-slate-100 border-slate-300")}
                      >
